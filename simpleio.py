...
def write_memory_to_file(A, filename, mode = 'w', title='test', complevel = None):
    """
    ...
    if mode == 'w':
        if np.iscomplexobj(B):
            h5file.createEArray("/","real", tb(), tuple(shape), filters = filters)
            h5file.createEArray("/","imag", tb(), tuple(shape), filters = filters)
        else:
            h5file.createEArray("/","real", tb(), tuple(shape), filters = filters)
...
def write_array(A, filename, mode = 'w', title='test', complevel=None):
    ...
    if mode == 'w':
        if (A.dtype == np.complex64):
            h5file.createEArray("/","array", tb(8), tuple(shape), filters = filters)
        elif (A.dtype == np.complex128):
            h5file.createEArray("/","array", tb(16), tuple(shape), filters = filters)
        else:
            h5file.createEArray("/","array", tb(), tuple(shape), filters = filters)
