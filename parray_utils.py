...
def get_fill_function(dtype, pitch = True):
    ...
    if pitch:
        ...
        func = SourceModule(
                ...
                options=["--ptxas-options=-v"]).get_function(name)
        func.prepare('iP'+np.dtype(dtype).char)
    return func
...
def get_addscalar_function(src_type, dest_type, pitch = True):
    ...
    if pitch:
        func = SourceModule(
                ...
                options=["--ptxas-options=-v"]).get_function(name)
        func.prepare('iiPiPi'+np.dtype(dest_type).char)
    else:
        func = SourceModule(
                ...
                options=["--ptxas-options=-v"]).get_function(name)
        func.prepare('PP'+np.dtype(dest_type).char+'i')
    return func
